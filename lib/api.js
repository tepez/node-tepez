...
class TepezApi {
    ...
    getEntry(entryId, fields) {
        Joi.assert(entryId, objectIdType.label('entryId'));
        ...
        return this._signinIfNeeded().then(() => {
            ...
            const requestOptions = {
                method: 'GET',
                url: `${this.options.baseUrl}/v1.0/entry/${entryId}`,
                time: true,
                json: true,
                qs: qs,
    ...
    saveNewEntry(formId, data) {
        Joi.assert(formId, objectIdType.label('formId'));
        Joi.assert(data, Joi.object().label('data'));
